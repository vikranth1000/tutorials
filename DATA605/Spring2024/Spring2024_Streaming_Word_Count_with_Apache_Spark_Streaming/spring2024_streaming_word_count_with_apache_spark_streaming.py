...
def process_stream(rdd):
    if not rdd.isEmpty():
        word_counts = (
            rdd.withColumn("word", F.explode(F.col("words_clean")))
            .groupBy("word")
            .count()
        ...
        word_counts_pd = word_counts.limit(
            10
        ).toPandas()  # Limit to top 10 words for visualization
...
"""
**In the section below, the SparkSession object (spark) is initialized to
enable the use of Spark SQL and the DataFrame API for processing structured
data.

By creating a unified entry point for interacting with different data
sources and executing SQL queries, SparkSession facilitates seamless
data manipulation within the Spark application, with the application
name set to "PythonWordCount".**
"""
...
data.show()
"""
**The below function we tokenize the lyrics data from the DataFrame using the
Tokenizer class from the pyspark.ml.feature module.

The Tokenizer splits the text into individual words or tokens, creating
a new column named "words_token" containing the tokenized lyrics. The
resulting DataFrame, containing the original rank along with the
tokenized lyrics, is then displayed to provide a glimpse of the
tokenized data.**
"""
...
"""
**In this final word count section, we compute the frequency of each word in
the cleaned lyrics data.

The DataFrame is transformed to explode the cleaned words column into
individual rows, grouped by word, and then aggregated to count the
occurrences of each word. The result is sorted in descending order based
on word count, providing insights into the most common words in the
lyrics dataset.**
"""
...
result.show()
"""
**In this section, we install the Pandas library using pip and import it as pd
for easy access.**
...
**The below visualization is the word counts obtained from the streaming
data analysis. The word counts DataFrame (result) is converted to a
Pandas DataFrame (result_pd) to facilitate visualization. Using
Matplotlib, we create a bar plot to display the top 10 words and their
respective counts. The plot provides a visual representation of the most
frequently occurring words in the streaming data, aiding in data
...
**This below section of code is performing text preprocessing and
analysis on lyrics data obtained from a CSV file. It returns these
metrics as word_count, average_word_length, most_common_word,
most_common_word_count, and unique_word_count.**
"""
...
(
    ...
    most_common_word_count,
    unique_word_count,
) = process_text(preprocessed_lyrics)

# Display results
print("Total Word Count:", word_count)
print("Average Word Length:", average_word_length)
print(
    "Most Common Word:", most_common_word, "(Count:", most_common_word_count, ")"
)
print("Number of Unique Words:", unique_word_count)
"""
**This section of the code defines a function visualize_results to create a bar
plot visualizing various text analysis metrics.**
"""
...
"""
**The below function, we create a DStream (Discretized Stream) by reading text
files from a specified directory.

The textFileStream method of the StreamingContext (ssc) is used to
create the stream, with the directory path provided as the argument.
This enables the streaming application to ingest and process data from
the text files in real-time, allowing for continuous analysis of the
...
"""
**Followed by the below code, we define a process to handle each RDD (Resilient
Distributed Dataset) in the stream.

The foreachRDD method is used to apply the process_stream function to
each RDD in the stream. This function processes the streaming data,
extracting word counts, and visualizing the results. By iterating over
each RDD in the stream, we ensure that the processing logic is applied
to every batch of data received, enabling real-time analysis of the
streaming data.**
"""
...
"""
In the below process, we initiate the streaming process by calling the start
method on the StreamingContext object (ssc).

Additionally, we use the awaitTermination method to instruct the program
to wait until the streaming process is terminated or manually stopped.
"""
