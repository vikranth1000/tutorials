...
_DAG_DESCRIPTION = "Download Coingecko data every hour and save to Postgres"
...
def bridge(data_interval_start):
    ...
    logging.info(
        "Setting Airflow Variable from_timestamp1 to " + str(utc_time_start)
    )
...
downloading_task = BashOperator(
    ...
    env={
        "pythonpath": "{{ var.value.get('pythonpath') }}",
        "from_timestamp": "{{ var.value.get('from_timestamp1') }}",
        "to_timestamp": "{{ var.value.get('to_timestamp1') }}",
    },
    dag=dag,
)
